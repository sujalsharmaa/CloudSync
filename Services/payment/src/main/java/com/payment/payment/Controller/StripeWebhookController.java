    ...
    public ResponseEntity<String> handleStripeWebhook(
        ...
        if ("checkout.session.completed".equals(event.getType())) {
            ...
            if (stripeObject instanceof Session) {
                // 1. Get the Session ID from the event payload
                String sessionId = ((Session) stripeObject).getId();

                try {
                    // 2. REFATCH: Retrieve the full session from Stripe (As per Example)
                    // This ensures data integrity and allows expanding fields like line_items
                    SessionRetrieveParams params = SessionRetrieveParams.builder()
                            .addExpand("line_items") // Matches the example's expansion logic
                            .build();
                    ...
                    // 3. Process the fully retrieved session
                    stripeService.handleSuccessfulPayment(session);
